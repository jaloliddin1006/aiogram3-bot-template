...
@router.message(CommandStart(), IsAdmin(207936288), IsPrivateChat())
...
@router.message(Command(commands=['rn', 'random']), CheckForDigit())
async def ping_pong(message: Message, command: CommandObject):
    a, b = list(map(int, command.args.split("-")))    
    rnum = random.randint(a, b)
    await message.answer(f'Random number: {rnum}', CheckForDigit())


@router.message(F.text.lower() == 'orqaga', CheckForDigit())
